👋 Hi, I’m Simamnkele

🎓 Bachelor of Commerce in Information Systems & Insurance and Risk Management (Completed)

💻 Aspiring Systems Analyst / Systems Developer

📍 South Africa

I’m passionate about designing, analysing, and building information systems that solve real-world problems. I enjoy working at the intersection of business processes and technology, translating requirements into practical, scalable system solutions.

🔍 What I’m Interested In

- Systems Analysis & Design
- Business Process Modelling
- Web Application Development
- Databases & Data Management
- Requirements Gathering & Documentation
- Improving system efficiency and user experience

🛠️ Technical Skills

- Languages & Technologies
- HTML, CSS, JavaScript
- C# (.NET, ASP.NET MVC)
- SQL
- Git & GitHub
- Concepts & Tools
- Systems Development Life Cycle (SDLC)
- UML Diagrams (Use Case, ERD, Sequence Diagrams)
- Database Design & Normalisation
- Agile & Waterfall methodologies
- Visual Studio / Visual Studio Code

📂 Current Projects
🚧 Actively building and improving my portfolio

  - Food Ordering Web Application
    ASP.NET MVC, C#, SQL
      - Customer-facing food ordering system
      - Admin dashboard for managing products and orders
      - Focus on system structure, data flow, and usability

  - Academic & Practice Projects
      - Systems analysis documentation
      - Business case analysis
      - Process modelling and system design

👉 More projects coming soon as I expand my development and analysis skills.
🌱 What I’m Currently Learning

- Advanced ASP.NET MVC patterns
- Clean architecture & best practices
- System documentation for real-world projects
- Bridging business requirements with technical solutions

🎯 Career Goals
Short-term:
  - Build strong, practical projects showcasing systems analysis and development
  - Gain experience in real-world system design and implementation

Long-term:
  - Work as a Systems Analyst or Systems Developer
  - Contribute to impactful systems that improve organisational decision-making

🤝 Let’s Connect
💼 LinkedIn: (www.linkedin.com/in/simamnkele-mthandekiso-168a5927b)
📧 Email: (mthandekisosims@gmail.com)

I’m always open to learning, collaboration, and opportunities to grow in the tech space.

<!---
simamnkele-m/simamnkele-m is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
